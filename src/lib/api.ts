...
// Справочник предметов меняется примерно никогда (в отличие от
// прогресса/профиля, которые обновляются после каждого решённого
// задания) — 30-секундный TTL по умолчанию в usePublicData тут только
// зря заставляет перезапрашивать один и тот же список при быстрой
// навигации Предметы -> Тема -> Решение -> Пробник. 5 минут — если
// список предметов реально поменяется на бэкенде, пользователь всё
// равно увидит актуальные данные при следующем заходе на сайт/перезагрузке.
export const SUBJECTS_CACHE_TTL_MS = 5 * 60_000;
